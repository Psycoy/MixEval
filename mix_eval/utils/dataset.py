...
def get_eval_dataset(args):
    if (
        ...
        or args.split == "audio_close_freeform_hard"
    ):
        return EvalDatasetCloseended(args)
    else:
        raise ValueError(
            f"Split {args.split} not supported in {get_eval_dataset.__name__}."
        )
...
class EvalDatasetCloseended(Dataset):
    def __init__(self, args):
        ...
        if args.split == "close_freeform":
            ...
            data_path_multiplechoice = os.path.join(
                version_dir, "mixeval/multiple-choice.json"
            )
            ...
            data_path_freeform_hard = os.path.join(
                version_dir, "mixeval-hard/free-form.json"
            )
            ...
            data_path_multiplechoice_hard = os.path.join(
                version_dir, "mixeval-hard/multiple-choice.json"
            )
            with open(data_path_multiplechoice_hard, "r") as f:
                ...
                for id, d in data.items():
                    ...
                    d["id"] = id
                    raw_inputs.append(d)
        elif args.split == "audio_close_freeform_hard":
            print("Loading audio close-ended freeform hard data.")
            data_path = os.path.join(
                version_dir, "mixeval-hard-audio-in/close_freeform_hard.jsonl"
            )
            ...
            raise ValueError(
                f"Split {args.split} not supported in {self.__class__.__name__}"
            )
        ...
        raw_inputs = sorted(
            raw_inputs,
            key=lambda x: len(nltk.word_tokenize(x["formated_input"])),
            reverse=True,
        )
